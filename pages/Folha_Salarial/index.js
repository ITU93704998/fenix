...
import DadosSomar from './dados_somar';
...
function Folha_Salarial() {
  ...
  let objetosFiltradosAnoMes = gastos.filter(
    item => item.data.substr(3) == mesPesquisa + '/' + ano,
  );
        ...
        <View
          ...
          <View style={{width: '90%', marginTop: 20}}>
            <Picker
              ...
              onValueChange={(itemValue, itemIndex) => setRecebedor(itemValue)}>
              <Picker.Item label="Selecione funcionário:" value="0" />
              {dados.length > 0
                ? dados.map(item => (
                    <Picker.Item label={item.nome} value={item.uid} />
                  ))
                : ''}
            </Picker>
          </View>
          ...
          <View
            ...
            <View style={{flexDirection: 'row', justifyContent: 'flex-end'}}>
              <View
                ...
                <FlatList
                  data={objetosFiltradosAnoMes.filter(
                    items => items.recebedor == item.uid,
                  )}
                  showsVerticalScrollIndicator={false}
                  style={{}}
                  renderItem={({item}) => (
                ...
                <View
                  ...
                  <Text style={{color: 'green', fontWeight: 'bold'}}>
                    + R$
                    <DadosSomar nome={item.nome} mess={mesPesquisa} somar={0} />
                  </Text>
                </View>
              </View>
            </View>
            ...
            <View
              ...
              <Text
                style={{color: '#696969', fontWeight: 'bold', fontSize: 15}}>
                R$
                <DadosSomar
                  nome={item.nome}
                  mess={mesPesquisa}
                  somar={
                    item.salario -
                    objetosFiltradosAnoMes
                      .filter(items => items.recebedor == item.uid)
                      .reduce((somatoria, quantidade) => {
