...
import { View, StyleSheet, TextInput, KeyboardAvoidingView, Text, FlatList, Modal , TouchableOpacity} from "react-native";
...
const List = () => {
  ...
  let producaoFilterUser = filterpro.filter( item => item.costureiro ==  (uid == 'v7RPkSgKE7PwUo6fnyDbR3JSFU23' ? pesquisaGerencia : costureiroUid))
  var TotalGastosFalarial = gastos.reduce((somatoria, quantidade) => { return somatoria + parseInt(quantidade.valor)}, 0);

  let objetosFiltradosAnoMes = producaoFilterUser.filter( item => item.data_costura.substr(3) == pesquisames+'/'+pesquisaano)
  ...
  let objetosFiltrados = objetosFiltradosAnoMes.filter(result => {
    if(pesquisadi == '' & pesquisadf == ''){
      ...
      return result.data_costura.substr(0, 2) >= pesquisadi && result.data_costura.substr(0, 2) <= pesquisadf;
    }
...
let totalforros = objetosFiltrados.filter(item => item.modelo == 'ForroPorta')
...
var totalcomissao = totalcapasInt * valorcapa + totaltapetesInt * valortapete + TotalforrosInt * valorforro;
...
const myFunction1 = async () => {
  ...
  return (
    ...
    <View style={{}}>
      ...
      <View style={{flexDirection: 'row', justifyContent:'space-around', alignItems: 'center', backgroundColor: '#027381', padding: 20,height: 130,}}>
     ...
     {uid === 'v7RPkSgKE7PwUo6fnyDbR3JSFU23' ?
        <View style={{width: '26%', flexDirection: 'row', justifyContent: 'center', alignItems: 'center'}}>
          <TextInput
            style={{ backgroundColor: "white",height: 33, padding: 5, borderRadius: 5, width: '95%', color: '#4B4B4B'}}
...
<View style={{backgroundColor: 'white', flexDirection: 'row', justifyContent:'center', alignItems: 'center', borderRadius: 5,width: uid == 'v7RPkSgKE7PwUo6fnyDbR3JSFU23' ? '70%' : '100%' }}>
      <View style={{width: '15%', flexDirection: 'row', justifyContent: 'center', alignItems: 'center',}}>


      <TextInput
        style={{ backgroundColor: 'white',height: 30, padding: 5, borderRadius: 5,width: '95%', color: '#4B4B4B'}}
        placeholder="00"
      ...
      <View ><Icon name="keyboard-arrow-right" size={20} color="#696969" /></View>
      <View style={{width: '15%', flexDirection: 'row', justifyContent: 'center', alignItems: 'center'}}>
      ...
      <View style={{}}><Text style={{color: '#696969', fontSize: 25}}>/</Text></View>
      <View style={{width: '15%', flexDirection: 'row', justifyContent: 'center', alignItems: 'center'}}>

          <TextInput
            style={{ backgroundColor: "white",height: 30, padding: 5, borderRadius: 5, color: '#4B4B4B'}}
            placeholder="00"
      ...
      <View style={{flexDirection: 'row', justifyContent: 'center', width: '100%'}}>
        <View style={{flexDirection: 'row', justifyContent: 'space-around', width: '90%', height: 80, backgroundColor: "#FF7751", marginTop: -40, borderRadius: 10, elevation: 5}}>
            <View style={{flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}}>
            <Text style={{fontSize: 25, color:'#fff', fontWeight: 'bold'}}><Text style={{fontSize: 12, fontWeight: 'bold'}}>R$ </Text>{totalcomissao}</Text>
              <Text style={{color: '#fff', fontSize: 10}}>Comissão</Text>
            </View>
            <View style={{flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}}>
            <Text style={{fontSize: 25, color:'#ffff', fontWeight: 'bold'}}>{totalcapasInt}</Text>
              <Text style={{color: '#fff', fontSize: 10}}>Capas</Text>
            </View>
            <View style={{flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}}>
              <Text style={{fontSize: 25, color:'#ffff', fontWeight: 'bold'}}>{totaltapetesInt}</Text>
              <Text style={{color: '#fff', fontSize: 10}}>Tapetes</Text>
            </View>
            <View style={{flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}}>
              <Text style={{fontSize: 25, color:'#fff', fontWeight: 'bold'}}>{TotalforrosInt}</Text>
              <Text style={{color: '#fff', fontSize: 10}}>Forros</Text>
            </View>

            <View style={{flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}}>
              <Text style={{fontSize: 25, color:'#ffff', fontWeight: 'bold'}}>{totaloutrosInt}</Text>
    ...
    <TouchableOpacity style={{marginRight: 10, flexDirection: 'row', justifyContent:'center', alignItems: 'center'}} onPress={() => {setValorSalario(!valorSalario) , setPesquisadi('01'), setPesquisadf('31') } }>
      <Text style={{fontWeight: '500'}}> Ver Folha salarial</Text>
      </TouchableOpacity>
    </View>
    : <View></View> }
    ...
    {valorSalario == true ?
        ...
        <View style={{padding: 20, marginTop: -20}} >
          <View style={{width: '100%',backgroundColor: '#ffff', padding: 10, borderRadius:5, elevation: 5,}}>
              <Text style={{fontSize: 17, fontWeight: 'bold'}}>{costureiroUid}</Text>
              ...
              <View style={{flexDirection: 'row', justifyContent: 'flex-end'}}>
                  <View style={{flexDirection:  'column', justifyContent:'space-evenly', width: '90%', marginTop: 5}}>
                      <View style={{width: '100%', flexDirection: 'row', justifyContent: 'space-between'}}>
                          <Text>Salário</Text>
                          <Text style={{color: 'green', fontWeight: '500'}}>+ R${vsalerio}</Text>
                      </View>
                      <FlatList
                      data={gastos}
                      ...
                      <View style={{width: '100%', flexDirection: 'row', justifyContent: 'space-between'}}>
                          <Text>Comissão</Text>
                          <Text style={{color: 'green', fontWeight: '500'}}>+ R${totalcomissao}</Text>
                      </View>
                  </View>
              </View>
              ...
              <View style={{width: '100%', flexDirection: 'row', justifyContent: 'space-between', marginTop: 5}}>
                          ...
                          <Text style={{color: '#696969', fontWeight: '500', fontSize:15}}>R$ {vsalerio + totalcomissao - TotalGastosFalarial}</Text>
              </View>
          </View>
        </View>
      : <View></View>
...
<View style={{padding: 10, marginTop:uid === 'v7RPkSgKE7PwUo6fnyDbR3JSFU23' ? 1 : -10 }}>
<FlatList
      data={objetosFiltrados.sort(compare)}
      showsVerticalScrollIndicator={false}
      style={{ height: uid === 'v7RPkSgKE7PwUo6fnyDbR3JSFU23' ? '77%' : '72%'}}
      renderItem={({ item }) => (
