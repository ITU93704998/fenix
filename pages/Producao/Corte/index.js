...
import { View, StyleSheet, TextInput, Button, Text, Modal, Alert, FlatList, TouchableOpacity, ActivityIndicator } from "react-native";
...
const UselessTextInput = ({navigation}) => {
  ...
  return (
     <View style={{flex:1, backgroundColor: '#027381'}}>
      ...
      >
        <View style={{ backgroundColor: "#027381", flex: 1,flexDirection: 'column', justifyContent: 'center', alignItems: 'center'  }}>
          ...
          <TouchableOpacity style={{width: '90%',marginTop: 10}}  onPress={() => addProducao()}>
            <View style={{backgroundColor: "#FF7751", height: 40, flexDirection: 'row',borderRadius: 10, justifyContent: 'center', alignItems: 'center'}}>
            <ActivityIndicator style={{marginRight: 5}} animating={valorindicator} size="small" color="#fff" />
      ...
      <View style={{flexDirection: 'row', justifyContent: 'space-between',marginTop: 20,}}>
        ...
        <TouchableOpacity onPress={() => navigation.navigate('Moldes')} style={{padding: 7, backgroundColor: '#FF7751', borderRadius: 6,paddingHorizontal: 30}}>
      ...
      renderItem={({ item }) => (
        <Ct nome={item.nomep} quantidade={item.quantidadep} plataforma={item.plataformap} id={item.key} modelo={item.modelo}/>
      )}
      />
