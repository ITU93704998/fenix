...
const CorteMesa = () => {
  ...
  const Pesquisar = async () => {
    if(pesquisa === ''){
      ...
      setProducaor(
        producaor.filter(item => (item.nomep.toLowerCase().indexOf(pesquisa.toLowerCase()) > -1))
      )
    }
  }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#027381', padding: 10}}>
    ...
    <FlatList
     ...
     style={{height: '92%', padding: 5}}
      ...
      renderItem={({ item }) => (
        <Ct  nome={item.nomep} quantidade={item.quantidadep} plataforma={item.plataformap} id={item.key} modelo={item.modelo} data_corte={item.data_corte} materiais_uso={item.materiais_uso}/>
      )}
    />
