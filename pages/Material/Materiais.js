...
import { Text, View, TextInput , TouchableOpacity, Modal, StyleSheet, Button, Alert, FlatList} from "react-native";
...
function Materiais({ navigation }) {
  ...
  const myFunction = async () => {
    if(pesquisa === ''){
      ...
      setEstoque(
        estoque.filter(item => (item.nome.toLowerCase().indexOf(pesquisa.toLowerCase()) > -1))
      )
    }
  };
  ...
  function AddEstoque(){
    ...
    .then(() => Alert.alert("Materiais", "Material adcionado!"),setVeri(!veri))
      ...
      >
        <View style={{ backgroundColor: "#027381", flex: 1,flexDirection: 'column', justifyContent: 'center', alignItems: 'center'  }}>
          ...
          <Picker
            ...
            <Picker.Item label="Courvin Programado" value="Courvin Programado" />
      ...
      renderItem={({ item }) => (
        ...
        <View style={{width: '100%', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center' ,}}>
          <View style={{width: '15%', flexDirection: 'column', justifyContent: 'space-around', alignItems: 'center'}}>
            <Text style={{fontSize: 30, color: '#565050',fontWeight: 'bold'}}>{item.quantidade}</Text>
          ...
          <View style={{marginLeft: 10, width: '85%',  flexDirection: 'column', justifyContent: 'center',}}>
            ...
            <Text style={{fontSize: 10,color: '#696969'}}>{item.fabricante}</Text>
           </View>
           
          
        </View>
        <View style={{flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', padding: 5, marginTop: 5}}>    
          <TouchableOpacity style={{height: 35,alignItems: 'center', justifyContent: 'space-around', alignItems: 'center', width: '35%', backgroundColor: '#FF7751', borderRadius: 5}} onPress={() => Retirar(item.key, item.quantidade, item.modelo, item.nome) }>
           <Text style={{fontWeight: 'bold', color: '#ffff'}}>Retirar Material</Text>
          </TouchableOpacity>

          <View style={{width: '60%', backgroundColor: '#dcdcdc', padding: 3, flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', borderRadius: 6}}>
            <TextInput
              style={{ backgroundColor: "#fff", height: 29, padding: 5, borderRadius: 5, width: '45%' , color: '#696969'}}
              placeholder="0"
              ...
              keyboardType="numeric"
            />
            <TouchableOpacity style={{height: 30,alignItems: 'center', justifyContent: 'space-around', alignItems: 'center', width: '45%', backgroundColor: '#FF7751', borderRadius: 5}} onPress={() => AdcionarMais(item.key, item.quantidade, quantidadeamais) }>
            <Text style={{fontWeight: 'bold', color: '#ffff'}}>Adcionar</Text>
            </TouchableOpacity>
          </View>
